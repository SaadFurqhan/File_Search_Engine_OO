...
from src.Drives.no_of_drives import *
import threading
from src.Logger.log import *
import string
...
def search():
    if request.method == 'POST':
        ...
        if searchDB == "on":
            if drives:
                result_from_db = []
                results = db.session.query(Model).filter(Model.searchTerm == search_term,
                                                         Model.pathSearched == drives).all()
                if len(results) == 0:
                    result_from_db.append(
                        "No match found! Please uncheck search from database, to search in local system.")
                for r in results:
                    result_from_db.append(r.result)
                object_result = {
                    "search_term": search_term,
                    "results": result_from_db,
                    "success_message": "Results are obtained from database! Total " + str(
                        len(results)) + " results are found.",
                    "directory": drives
                }
                return render_template("search.html", result=object_result)
                ...
                result_from_db = []
                results = db.session.query(Model).filter(Model.searchTerm == search_term,
                                                         Model.pathSearched == single_directory).all()
                if len(results) == 0:
                    result_from_db.append(
                        "No match found! Please uncheck search from database, to search in local system.")
                for r in results:
            ...
            if drives != "":
                ...
                if object_result.get('results') == 0:
                    ...
                    thread = threading.Thread(target=add_to_db,
                                              args=(object_result.get('results'), search_term, drives),
                                              daemon=True).start()
            if single_directory != "":
                ...
                if object_result.get('results') == 0:
                    ...
                    thread = threading.Thread(target=add_to_db,
                                              args=(object_result.get('results'), search_term, single_directory),
                                              daemon=True).start()
            return render_template("search.html", result=object_result)
